...
function signup_another_blog($blogname = '', $blog_title = '', $errors = '') {
	...
	/* rc_corephp - Changed verbage depending if there is a primary blog set or not */
	if ( $current_user->primary_blog ) {
		echo '<h2>' . sprintf( __( 'Get <em>another</em> %s site in seconds' ), $current_site->site_name ) . '</h2>';
	} else {
	echo '<h2>' . sprintf( __( 'Get a %s site in seconds' ), $current_site->site_name ) . '</h2>';
	}
	...
	<?php /* rc_corephp - Fixed the path to the signup form */ ?>
	<form id="setupform" method="post" action="<?php echo site_url('wp-signup.php'); ?>">
