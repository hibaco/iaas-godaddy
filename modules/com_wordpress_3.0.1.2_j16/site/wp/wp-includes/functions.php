...
function update_site_option( $option, $value ) {
	...
	if ( !is_multisite() ) {
		...
		/* rc_corephp - Fixed bug where an original $value of 0 would not be created in DB */
		if ( ( $value || 0 == $value ) && !$wpdb->get_row( $wpdb->prepare( "SELECT meta_value FROM $wpdb->sitemeta WHERE meta_key = %s AND site_id = %d", $option, $wpdb->siteid ) ) )
