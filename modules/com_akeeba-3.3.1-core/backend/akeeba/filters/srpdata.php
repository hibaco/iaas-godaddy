 ...
 * @version $Id: srpdata.php 632 2011-05-22 20:44:46Z nikosdion $
 */
...
class AEFilterSrpdata extends AEAbstractFilter
{
	...
	protected function is_excluded_by_api($test, $root)
	{
		...
		if(in_array($barename, $this->params->customtables)) return false;
		
		// Does it start with the name prefix?
		if(strpos($barename, $this->params->name.'_') === 0) return false;
		
		// Does it start with any of our extra prefixes?
		foreach($this->params->extraprefixes as $prefix) {
			if(substr($prefix,-1) != '_') $prefix .= '_';
			if(strpos($barename, $prefix) === 0) return false;
		}
