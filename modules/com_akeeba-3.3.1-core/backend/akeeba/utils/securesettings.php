 ...
 * @version $Id: securesettings.php 409 2011-01-24 09:30:22Z nikosdion $
 * @since 3.2
 */
...
class AEUtilSecuresettings
{
	...
	public static function encryptSettings($settingsINI, $key = null)
	{
		...
		if(!self::supportsEncryption()) return $settingsINI;
		// Does any of the preferred encryption engines exist?
		$encryption = self::preferredEncryption();
		if(empty($encryption)) return $settingsINI;
		// Do we have a non-empty key to begin with?
		if(empty($key))	$key = self::getKey();
		if(empty($key)) return $settingsINI;
