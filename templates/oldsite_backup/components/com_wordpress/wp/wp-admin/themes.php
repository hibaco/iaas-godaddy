...
if ( current_user_can('install_themes') ) {
	?>
	Just a friendly warning:<br />
	If you install and use any theme aside from the Twenty Ten theme that comes pre-installed, only that theme will display when the blog is viewed on the website. The reason for this is because we don't want your page to invalidate when search engines crawl it.
	<br /><br />
	If your blog displays two different templates (joomlas and wordpress), it will have duplicate html, head, title and other important tags that should only display once per page. The Twenty Ten theme comes modified to work inside of Joomla, but any other theme won't be like this.
	<?php
}
?>
...
<?php if ( !empty($theme_name) ) :
	...
	/* rc_corephp - Added option query argument */
	global $component_real_name;
	$preview_link = htmlspecialchars( add_query_arg( array('option' => $component_real_name, 'preview' => 1, 'template' => $template, 'stylesheet' => $stylesheet, 'TB_iframe' => 'true' ), $preview_link ) );
