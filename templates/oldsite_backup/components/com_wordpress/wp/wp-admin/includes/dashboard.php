...
function wp_dashboard_setup() {
	...
	if ( !isset( $widget_options['dashboard_primary'] ) ) {
		...
		$widget_options['dashboard_primary'] = array(
			'link' => apply_filters( 'dashboard_primary_link',  __( 'http://www.corephp.com/blog/' ) ),
			'url' => apply_filters( 'dashboard_primary_feed',  __( 'http://www.corephp.com/blog/feed/' ) ),
			'title' => apply_filters( 'dashboard_primary_title', __( '\'corePHP\' Blog' ) ),
			'items' => 2,
			'show_summary' => 1,
			'show_author' => 0,
			'show_date' => 1,
...
?>
		...
		<div id="comment-<?php echo $comment->comment_ID; ?>" <?php comment_class( array( 'comment-item', wp_get_comment_status($comment->comment_ID) ) ); ?>>
			...
			<?php echo getSocialAvatar( $comment, 50 ); ?>
