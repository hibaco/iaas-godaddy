...
function list_core_update( $update ) {
	...
	if ( 'development' == $update->response ) {
		...
		$message = __('You are using a development version of WordPress.  You can update to the latest version manually:');
		$download = __('Download latest');
	} else {
		if ( $current ) {
			/* rc_corephp - Changed verbiage */
			$message = sprintf(__('You have the latest version of WordPress. You do not need to update. However, if you want to re-install version %s, you can do so manually:'), $version_string);
			$submit = __('Download latest');
