...
function get_search_form($echo = true) {
	...
	if ( ( $itemid = j_get_itemid() ) ) {
		$itemid = '<input type="hidden" value="'.$itemid.'" name="Itemid" />';
	}

	/* rc_corephp - Added class="button" */
	$form = '<form role="search" method="get" id="searchform" action="' . home_url( '/' ) . '" >
	<div><label class="screen-reader-text" for="s">' . __('Search for:') . '</label>
	<input type="text" class="inputbox" value="' . get_search_query() . '" name="s" id="s" />
	<input type="hidden" value="'.$component_real_name.'" name="option" />
	'.$itemid.'
