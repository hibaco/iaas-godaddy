...
function network_step1( $errors = false ) {
	...
	if ( isset( $_POST['subdomain_install'] ) ) {
		...
		if ( $got_mod_rewrite = got_mod_rewrite() ) // dangerous assumptions
			/* rc_corephp - Commenting this out as it could potentially confuse people */
			...
			/* rc_corephp - This is to not disturb the if statement */
			echo '';
		elseif ( $is_apache )
...
?>
		<?php
		/* rc_corephp - Include needed classes */
		jimport( 'joomla.filesystem.folder' );
		jimport( 'joomla.filesystem.file' );
		?>
		<ol>
			<?php
			/* rc_corephp - Attempt to create blogs.dir folder */
			$blogs_id = WP_CONTENT_DIR .DS. 'blogs.dir';
			// If unable to create then display instructions
			if ( !is_dir( $blogs_dir ) && !JFolder::create( $blogs_id ) ) { ?>
			<li><p><?php
			...
			?></p></li>
			<?php } // End for creating blogs.dir folder ?>
			<?php
			/* rc_corephp - Attempt to write new configs to wp-config.php file */
 			$config_file = ABSPATH . 'wp-config.php';
	 		$file_contents = JFile::read( $config_file );
			$string_to_find = "define('WP_ALLOW_MULTISITE', true);";
			...
			<?php
			$string_to_write = ob_get_clean();

			if ( false == $file_contents || defined( 'WP_ALLOW_MULTISITE' ) ) {
			?>
			<li><p><?php printf( __( 'Add the following to your <code>wp-config.php</code> file in <code>%s</code> <strong>above</strong> the line reading <code>/* That&#8217;s all, stop editing! Happy blogging. */</code>:' ), ABSPATH ); ?></p>
				<?php /* rc_corephp - Let the user know to remove the previous define */ ?>
				<p>*** On the same file wp-config.php, search for this string and delete it: "define('WP_ALLOW_MULTISITE', true);"</p>
...
RewriteRule ^ - [L]';
		...
		?>
		<li><p><?php printf( 'Create an <code>.htaccess</code> file in <code>%s</code>, and add the following rules to it:', ABSPATH ); ?></p>
